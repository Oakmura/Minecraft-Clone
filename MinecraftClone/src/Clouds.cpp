...
Clouds::Clouds()
{
    ...
    std::vector<uint8_t> cloudData(WORLD_AREA * CHUNK_SIZE * CHUNK_SIZE, 0);

    for (int x = 0; x < WORLD_WIDTH * CHUNK_SIZE; ++x)
    {
        for (int z = 0; z < WORLD_DEPTH * CHUNK_SIZE; ++z)
        {
            if (simplexNoise.eval(0.13 * x, 0.13 * z) >= 0.2)
            {
                cloudData[x + WORLD_WIDTH * CHUNK_SIZE * z] = 1;
            }
        }
    }

    mVertices.reserve(WORLD_AREA * CHUNK_AREA * 6 * 3);
    mIndices.reserve(WORLD_AREA * CHUNK_AREA * 6 * 3);
...
{
    int y = CLOUD_HEIGHT;
    int width = WORLD_WIDTH * CHUNK_SIZE;
    int depth = WORLD_DEPTH * CHUNK_SIZE;
